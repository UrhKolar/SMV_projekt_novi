...
<body>
    <div class="dashboard-wrap">
        ...
        <div class="dashboard-grid" style="display:flex;justify-content:center;gap:20px;flex-wrap:wrap;">
            <?php if ($mojiPredmeti): ?>
                <?php foreach ($mojiPredmeti as $p): ?>
                    <div class="dashboard-card" style="width:280px; border-radius: 20px; background: rgba(255, 255, 255, 0.6); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="dashboard-card" style="width:280px;text-align:center;color:#6c757d; border-radius: 20px; background: rgba(255, 255, 255, 0.6); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);">
        ...
        <div class="dashboard-card" style="margin-bottom:16px; border-radius: 20px; background: rgba(255, 255, 255, 0.6); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);">
        ...
        <div class="dashboard-grid" id="subjects-list" style="display:flex;justify-content:center;gap:20px;flex-wrap:wrap;">
            <?php foreach ($vsiPredmeti as $p): ?>
                ...
                <div class="dashboard-card subject-card" style="width:280px; border-radius: 20px; background: rgba(255, 255, 255, 0.6); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);" data-search="<?= strtolower($p['naziv'] . ' ' . ($p['kratica'] ?? '') . ' ' . ($p['opis'] ?? '')) ?>">
                    <div style="margin-bottom:8px;font-weight:700;color:#2c3e50;font-size:1.1rem;">
                        <?= htmlspecialchars($p['naziv']) ?><?= $p['kratica'] ? ' <span style="color:#6c757d;">(' . htmlspecialchars($p['kratica']) . ')</span>' : '' ?>
                        <?php if ($isEnrolled): ?>
